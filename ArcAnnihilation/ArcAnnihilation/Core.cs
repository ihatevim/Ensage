...
namespace ArcAnnihilation
{
    ...
    public class Core
    {
        ...
        private static uint GetComboOrderId(Item y, bool byIllusion)
        {
            //Print(Menu.Item("itemTempest").Name);
            if (Menu.Item(byIllusion ? "customOrderIdTempest" : "customOrderIdHero").GetValue<bool>())
        ...
        private static void InitMenu()
        {
            if (_firstTime)
            {
                ...
                itemHero.AddItem(new MenuItem("customOrderIdHero", "Use Custom OrderId").SetValue(false));
                ...
                itemTempest.AddItem(new MenuItem("customOrderIdTempest", "Use Custom OrderId").SetValue(false));
                itemTempest.AddItem(
                ...
                drawItems.AddItem(new MenuItem("Draw.OrderId.Type", "Draw Type: New").SetValue(true))
                    .SetTooltip("use new way for drawing current OrderIds").ValueChanged += (sender, args) =>
                    {
                ...
                //il.AddItem(new MenuItem("OrderIdList", "Use OrderId list").SetValue(false));
                Menu.AddItem(
                    new MenuItem("OrderId", "Clone OrderId Selection").SetValue(
        ...
        **/
        private static void Player_OnExecuteAction(Player sender, ExecuteOrderEventArgs args)
        {
            ...
            if (Menu.Item("OrderId").GetValue<StringList>().SelectedIndex == (int)OrderIds.Monkey && !Menu.Item("AutoPush.Enable").GetValue<KeyBind>().Active)
            {
        ...
        **/
        ...
        private static void DoCombo(Hero me, Hero target)
        {
            ...
            bool dagger;
            double targetHull = target.HullRadius;
            if (Menu.Item("OrderId").GetValue<StringList>().SelectedIndex == (int) OrderIds.Caster &&
                !Menu.Item("hotkeyClone").GetValue<KeyBind>().Active)
            {
                foreach (var hero in Objects.Tempest.GetCloneList(me))
                {
                ...
                inventory.Where(
                    x =>
                        ...
                        ((x.CastRange == 0 &&
                         ...
                         /*x.CastRange+50 >= distance*/x.CanHit(target))).OrderByDescending(y => GetComboOrderId(y, byIllusion));
